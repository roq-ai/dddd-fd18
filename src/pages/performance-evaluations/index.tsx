...
type ColumnType = ColumnDef<PerformanceEvaluationInterface, unknown>;

interface PerformanceEvaluationListPageProps {
...
export function PerformanceEvaluationListPage(props: PerformanceEvaluationListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<PerformanceEvaluationInterface>>(
    () => `/performance-evaluations?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: PerformanceEvaluationInterface) => {
    if (hasAccess('performance_evaluation', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/performance-evaluations/view/${row.id}`);
    }
  };
  ...
  return (
    <Box p={4} rounded="md" shadow="none">
      <AccessInfo entity="performance_evaluation" />
